...
public class ReflectionUtil {
	...
	public static Object getValue(Object object, String propertyName) {
		...
		Object valor = null;
		if (object != null) {
			log.log(Level.FINE, "Object is not null");
			...
			if (!done) {
				...
				if (!propertyName.contains(".")) {
                                    log.log(Level.FINE, "propertyName não contém ponto");
	 ...
	 */
		...
		for (Class<?> clazz = type; clazz != Object.class; clazz = clazz.getSuperclass()) {
			try {
                                log.log(Level.FINE, "Tentando encontrar o método na classe %s", clazz.getName());
				method = clazz.getDeclaredMethod(methodName);
				log.log(Level.INFO, "Método %s encontrado na classe %s", new Object[] {methodName, clazz.getName()});
				break;
			} catch (Exception e) {
                            log.log(Level.FINE, "O método %s não existe na classe %s", new Object[] {methodName, clazz.getName()});
			}
		}
	...
	private static Field getClassField(Class<?> type, String propertyName) {
		...
		// iterando sobre a estrutura da classe e das superclasses
		for (Class<?> clazz = type; clazz != Object.class; clazz = clazz.getSuperclass()) {
                    log.log(Level.FINER, "clazz = " + clazz);
			try {
				atributo = clazz.getDeclaredField(propertyName);
                                log.log(Level.FINER, "atributo = " + atributo);
				break;
			} catch (Exception e) {
				log.log(Level.FINE, "O atributo %s não foi encontrado na classe %s", new Object[] {propertyName, clazz.getName()});
			}
		}
	...
	public static Class<?> getFieldType(Class<?> type, String propertyName) {
		...
		String[] path = propertyName.split("\\.");
                log.log(Level.FINER, "path.length = " + path.length);
		for (int i = 0; i < path.length; i++) {
			propertyName = path[i];
			PropertyDescriptor[] propDescs = PropertyUtils.getPropertyDescriptors(type);
                        log.log(Level.FINER, "propDescs.lenght = " + propDescs.length);
			for (PropertyDescriptor propDesc : propDescs)
				if (propDesc.getName().equals(propertyName)) {
                                    log.log(Level.FINE, "Nome da propriedade encontrado");
	...
	public static List<Field> getFields(Class<?> classe, Class<? extends Annotation> annotation) {
		...
		for (Class clazz = classe; clazz != Object.class; clazz = clazz.getSuperclass()) {
			try {
				...
				log.log(Level.FINE, "A anotação %s não foi encontrada na classe %s", new Object[] {annotation.getName(), clazz.getName()});
			}
		}
